...
class RoutePreferenceModel(nn.Module):
    def __init__(self, config: RoutePreferenceModelConfig):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(
                ROUTE_DERIVED_BRANCH_DIM + CONTEXT_CROSS_BRANCH_DIM + STOP_BRANCH_DIM + SEGMENT_BRANCH_DIM,
                FUSION_HIDDEN_DIM,
            ),
    ...
    def forward(
        ...
        hidden = self.encode(stop_flat, segment_flat, route_derived_vector, context_cross_vector)
